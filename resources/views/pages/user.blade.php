        ...
        <aside class="order-1 tablet:order-none border-l border-gray-700 tablet:w-[19rem]">
            <header class="flex items-center p-4">
                ...
                @if ($user->id == Auth::user()->id)
                    <a href="{{ url('/logout') }}" class="rounded-lg px-2 py-2 text-sm hover:bg-gray-700">
            ...
            @if (Auth::check() and Auth::user()->is_admin)
                <div class="p-4 flex flex-col gap-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <span
                            class="bg-purple text-white px-2 py-1 font-bold rounded-full text-sm ">{{ $user->status }}</span>
                    </div>
                </div>
            @endif
        </aside>

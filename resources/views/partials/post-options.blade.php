   ...
   @if (Auth::check() && Auth::user()->isAdmin())
      @if(!$post->is_omitted)
        ...
        <form method="POST" action="/news/{{ $post->id }}/omit">
         @csrf
         @method('PUT')
         <button class="text-gray-400 px-3 py-2 rounded-xl w-full flex items-center gap-2 text-sm hover:bg-gray-700"
     ...
     @else
        <form method="POST" action="/news/{{ $post->id }}/unomit">
         @csrf
         @method('PUT')
         <button class=" text-gray-400 text-sm px-3 py-2 rounded-xl w-full flex items-center gap-2 hover:bg-gray-700"
           type="submit"><svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24" fill="none"
            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"

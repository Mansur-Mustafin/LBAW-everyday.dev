        ...
        <div class="{{ count($comment->replies) == 0 ? '' : 'border-b border-solid border-gray-700'}} rounded-xl p-4">
            <div class="text-sm text-gray-400 flex gap-2">
                ...
                <div class="flex flex-col">
                    ...
                    <h3 class="text-xs text-gray-500">{{'@' . $comment->user->username}} · {{$comment->created_at->diffForHumans()}}</h3>
                </div>
            </div>
            <div class="mt-4">
                {{ $comment->content }}
...
@elseif($level > 2)
<a href="{{ url('/news/' . $post->id . '/single-thread/' . $parent->id) }}" class="mt-3 ml-3 text-sm hover:underline" id="showMore">
    Show {{ count($comment->parent->replies)}} more {{count($comment->parent->replies) == 1 ? 'answer' : 'answers' }}
</a>

@else
<div class="{{ count($comment->parent->replies) > 1 ? 'border-l border-opacity-50 border-solid border-gray-700' : ''}} flex flex-col ml-8">
    <div class="{{ $level == 1 && count($comment->replies) == 0 ? '' : 'border-b border-solid border-gray-700 rounded-bl-xl'}} {{count($comment->parent->replies) <= 1 ? 'border-l border-opacity-50 border-solid border-gray-700' : 'rounded-xl'}}  p-4">
        <div class="text-sm text-gray-400 flex gap-2">
            <img src="{$user->getProfileImage()}">
            <div class="flex flex-col">
                <h2 class="text-white text-sm font-semibold">{{$comment->user->public_name}}</h2>

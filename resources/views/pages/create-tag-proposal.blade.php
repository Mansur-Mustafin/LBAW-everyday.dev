...
<section
    ...
    <form method="POST" action="{{ url('/tag_proposals/create') }}" enctype="multipart/form-data"
        ...
        <div class="flex flex-col">
            ...
            <label class="font-bold text-sm">Why this tag?</label>
            <input name="description" class="rounded-2xl bg-input outline-none p-3" placeholder="Description*" value="">
            @error('description')
                <span class="text-red-400 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="flex gap-2 self-end">
            ...
            <button class="text-input bg-white font-bold rounded-xl px-6 py-2" type="submit">Create Tag Proposal</button>
        </div>

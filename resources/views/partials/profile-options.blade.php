<div id='profile-options-popup'
  class="bg-input w-48 absolute top-11 right-0 flex flex-col opacity-0 rounded-xl transition ease-out">
  ...
  @if (Auth::check() and (Auth::id() == $user->id))
    ...
    @csrf
    @method('PUT') --}}
    <button type="submit" data-url='{{url('')}}' data-user={{$user->id}}
      class="delete-account-button text-red-400 rounded-lg px-4 py-2 text-sm w-full text-start hover:bg-gray-700">
      Delete My Account
    </button>
    {{--
    </form> --}}
